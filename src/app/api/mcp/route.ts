...
const TOOLS: ToolDef[] = [
  ...
  {
    name: 'task_request',
    description: 'タスクを担当者に依頼し受諾待ちにします（依頼→承認ワークフロー開始）',
    inputSchema: {
      type: 'object',
      properties: {
        taskId: { type: 'string' },
        assigneeId: { type: 'string', description: '担当者のユーザーID' },
        dueDate: { type: 'string', description: '期日(ISO文字列)' },
        comment: { type: 'string', description: '依頼コメント' },
      },
  ...
  {
    name: 'task_accept',
    description: '依頼を受諾し対応中にします（担当者のみ）',
    inputSchema: { type: 'object', properties: { taskId: { type: 'string' } }, required: ['taskId'] },
    readOnly: false,
  },
  {
    name: 'task_decline',
    description: '依頼を辞退します（担当者のみ・コメント必須）',
    inputSchema: {
      type: 'object',
      properties: { taskId: { type: 'string' }, comment: { type: 'string', description: '辞退理由（必須）' } },
  ...
  {
    name: 'task_submit',
    description: '完了報告を出し承認待ちにします（担当者のみ）',
    inputSchema: { type: 'object', properties: { taskId: { type: 'string' } }, required: ['taskId'] },
    readOnly: false,
  },
  {
    name: 'task_approve',
    description: '完了報告を承認し完了にします（依頼者のみ）',
    inputSchema: { type: 'object', properties: { taskId: { type: 'string' } }, required: ['taskId'] },
    readOnly: false,
  },
  {
    name: 'task_send_back',
    description: '完了報告を差し戻します（依頼者のみ・コメント必須）',
    inputSchema: {
      type: 'object',
      properties: { taskId: { type: 'string' }, comment: { type: 'string', description: '差し戻し理由（必須）' } },
      required: ['taskId', 'comment'],
    },
    readOnly: false,
  },
  {
    name: 'task_cancel_request',
    description: '依頼を取り消し通常タスクに戻します（依頼者のみ）',
    inputSchema: { type: 'object', properties: { taskId: { type: 'string' } }, required: ['taskId'] },
    readOnly: false,
  },
  {
...
const TOOL_HANDLERS: Record<string, (params: any, ctx: ToolContext) => Promise<handlers.ToolResult>> = {
  ...
  task_request: handlers.handleTaskRequest,
  task_accept: handlers.handleTaskAccept,
  task_decline: handlers.handleTaskDecline,
  task_submit: handlers.handleTaskSubmit,
