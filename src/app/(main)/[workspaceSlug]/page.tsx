...
export default async function WorkspacePage({
  ...
  // 全クエリを可能な限り並列実行
  const [
    totalTasks, completedTasks, overdueTasks, inProgressTasks,
    statusGroups, priorityGroups,
    taskStats, overdueByProject, dueThisWeekByProject, nextDeadlineByProject,
    ...
    prisma.task.findMany({
      ...
      select: {
        ...
        startDate: true, dueDate: true, projectId: true,
      },
    ...
    prisma.task.findMany({
      ...
      select: {
        ...
        projectId: true, project: { select: { name: true, color: true } },
      },
    }),
  ]);
  ...
  const portfolioData = projects.map((project) => {
    ...
    return {
      ...
      total, completed, inProgress, todo, overdue, dueThisWeek,
      nextDeadline: nextDl?.dueDate ? { title: nextDl.title, date: nextDl.dueDate.toISOString() } : null,
    };
  });
    ...
    <>
      ...
      <div className="flex-1 overflow-auto bg-g-surface p-6">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <DashboardClient byStatus={byStatus} byPriority={byPriority} recentActivity={recentActivity} completionRate={completionRate} ganttProjects={ganttProjects} />
