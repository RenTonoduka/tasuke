...
export function StatusChart({ data, completionRate }: StatusChartProps) {
  ...
  return (
    <div className="rounded-lg bg-g-bg p-5 shadow-sm ring-1 ring-g-border">
      <h2 className="mb-4 text-sm font-semibold text-g-text">ステータス別タスク</h2>
      {chartData.length === 0 ? (
        <div className="flex h-48 items-center justify-center text-sm text-g-text-muted">データなし</div>
      ) : (
        <div className="relative flex flex-col items-center">
          ...
          <div className="absolute top-1/2 -translate-y-1/2 text-center">
            <p className="text-2xl font-bold text-g-text">{completionRate}%</p>
            <p className="text-xs text-g-text-muted">完了率</p>
          </div>
          <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1">
            {chartData.map((d) => (
              <div key={d.status} className="flex items-center gap-1.5 text-xs text-g-text-secondary">
                <span
