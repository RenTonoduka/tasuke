...
export function PriorityChart({ data }: PriorityChartProps) {
  ...
  return (
    <div className="rounded-lg bg-g-bg p-5 shadow-sm ring-1 ring-g-border">
      <h2 className="mb-4 text-sm font-semibold text-g-text">優先度別タスク</h2>
      {total === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={sorted} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="label" tick={{ fontSize: 12, fill: 'var(--g-text-secondary)' }} />
            <YAxis tick={{ fontSize: 12, fill: 'var(--g-text-secondary)' }} allowDecimals={false} />
            <Tooltip
