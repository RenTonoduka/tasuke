...
function WorkspaceDropZone({ ws }: { ws: { id: string; name: string; slug: string } }) {
  const { isOver, setNodeRef } = useDroppable({ id: `ws-drop-${ws.id}` });
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium border-2 border-dashed transition-colors',
        isOver
          ? 'border-[#4285F4] bg-[#4285F4]/20 text-[#4285F4] scale-[1.02]'
      ...
      <ScrollArea className="flex-1 px-2 py-2">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            {/* ドラッグ中: ワークスペースドロップゾーン */}
            {activeProject && workspaces.filter(ws => ws.slug !== currentWorkspaceSlug).length > 0 && (
              <div className="mt-3 space-y-1.5 border-t-2 border-dashed border-[#4285F4]/50 pt-3">
                <span className="block px-1 text-[11px] font-semibold text-[#4285F4]">
                  ↓ ワークスペースに移動
                </span>
                {workspaces.filter(ws => ws.slug !== currentWorkspaceSlug).map(ws => (
                  <WorkspaceDropZone key={ws.id} ws={ws} />
                ))}
              </div>
            )}
