...
function KeyBadge({ children }: { children: string }) {
  return (
    <kbd className="inline-flex h-5 min-w-5 items-center justify-center rounded bg-[#F1F3F4] px-1.5 font-mono text-[10px] font-medium text-[#5F6368]">
      {children}
    </kbd>
  );
}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-sm p-0">
        <DialogHeader className="border-b border-[#E8EAED] px-5 py-4">
          <DialogTitle className="text-sm font-semibold text-[#202124]">
            キーボードショートカット
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="divide-y divide-[#E8EAED]">
          {SHORTCUT_GROUPS.map((group) => (
            <div key={group.label} className="px-5 py-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#80868B]">
                {group.label}
              </p>
              <ul className="space-y-2.5">
                {group.items.map((item) => (
                  <li key={item.description} className="flex items-center justify-between gap-4">
                    <span className="text-sm text-[#5F6368]">{item.description}</span>
